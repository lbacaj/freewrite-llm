...
struct ContentView: View {
    ...
    @State private var showLLMResultSheet = false
    ...
    var body: some View {
                                            ...
                                            HStack {
                                                ...
                                                if hoveredEntryId == entry.id {
                                                    HStack(spacing: 8) {
                                                        ...
                                                        Button(action: {
                                                            processAllLLMPasses(entry: entry)
                                                        }) {
                                                            if processingLLMForEntry == entry.id {
                                                                ProgressView()
                                                                    .scaleEffect(0.7)
                                                            } else {
                                                                Image(systemName: "brain")
                                                                    .font(.system(size: 11))
                                                                    .foregroundColor(hoveredLLMButtonId == "\(entry.id)" ? 
                                                        ...
                                                        .onHover { hovering in
                                                            withAnimation(.easeInOut(duration: 0.2)) {
                                                                hoveredLLMButtonId = hovering ? "\(entry.id)" : nil
                                                            }
                                                            if hovering {
                                                                NSCursor.pointingHand.push()
                                                            } else {
                                                        ...
                                                        .disabled(processingLLMForEntry != nil)
        ...
        .sheet(isPresented: $showSettingsView) {
            SettingsView()
                .onDisappear {
                    // Check if user wants to download models after closing settings
                    if modelManager.downloadedModels.isEmpty {
                        showLLMDownloadSheet = true
                    }
                }
        }
        .onReceive(NotificationCenter.default.publisher(for: NSNotification.Name("ShowLLMDownloadSheet"))) { _ in
            showLLMDownloadSheet = true
        }
        .sheet(isPresented: $showLLMDownloadSheet) {
            LLMDownloadSheet(onModelSelected: .constant(false))
        }
        .sheet(isPresented: $showLLMResultSheet) {
            LLMResultSheet(
...
extension ContentView {
    ...
    private func processAllLLMPasses(entry: HumanEntry, forceReprocess: Bool = false) {
        ...
        do {
            let entryContent = try String(contentsOf: fileURL, encoding: .utf8)
            let cleanedContent = entryContent.replacingOccurrences(of: headerString, with: "").trimmingCharacters(in: .whitespacesAndNewlines)
            
            // Clear previous results
            llmResults.removeAll()
            llmProcessingStatus.removeAll()
            
            // Mark all passes as processing
            for pass in passStore.passes {
                llmProcessingStatus[pass.id] = true
            }
            
            // Show processing state
            processingLLMForEntry = entry.id
            showLLMResultSheet = true
            
            // Process all passes concurrently
            for pass in passStore.passes {
                Task {
                    do {
                        let result: String
                        ...
                        await MainActor.run {
                            llmResults[pass.id] = result
                            llmProcessingStatus[pass.id] = false
                            
                            // Check if all passes are done
                            let allDone = passStore.passes.allSatisfy { p in
                                llmProcessingStatus[p.id] == false
                            }
                            if allDone {
                        ...
                        await MainActor.run {
                            llmProcessingStatus[pass.id] = false
                            print("LLM processing error for pass \(pass.name): \(error)")
            ...
            processingLLMForEntry = nil
            print("Error reading entry: \(error)")
        }
    }
}
